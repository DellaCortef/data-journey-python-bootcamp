...
def retry(exception_to_check, tries=3, delay=1, backoff=2):
    """
    Decorator that retains the function multiple times in case of exception.
    ...
    :param delay: The initial time to wait between attempts.
        ...
        def wrapper_retry(*args, **kwargs):
            ...
            while _tries > 1:
                try:
                    ...
                    print(f"{func.__name__} failed, retrying in {_delay} seconds. Remaining attempts: {_tries - 1}")
                    time.sleep(_delay)
                    _tries -= 1
                    _delay *= backoff
            return func(*args, **kwargs)
